...
<div class="container-fluid">
    ...
    <tr>
        <h5 style="text-align:center;"> <p style="font-size:25px"> <strong>LIST OF ALL Residential PROPERTIES</strong></p></h5>
            ...
            <table class="table table-bordered">
                ...
                @foreach($data as $key => $value)
                    ...
                    <tr>
                        <td>
                            @if($value->customer)
                                <a href="javascript:void(0)" class="show_customer" >{{$value->customer->cnic}}<br>{{$value->customer->name}}</a>
                            @else
                            @endif
                        </td>
                        <td><a style="text-decoration: underline">{{$value->customer->designation}}</a></td>
                        <td>{{$value->coveredarea}}</td>
                        <td>{{$value->customer->organization()->name ?? ""}}</td>
